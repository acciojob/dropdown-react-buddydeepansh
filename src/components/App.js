...
function App() {
  ...
  const handleStates = (e) => {
    console.log(StateRef.current.selectedIndex);
    setstateSelected(StateRef.current.selectedIndex);
    setcitySelected(0);
  };
  const handleCities = (e) => {
    setcitySelected(cityRef.current.selectedIndex);
    console.log(cityRef.current.selectedIndex);
    setLandmarkSelected(0);
  };
  const handleLandmarks = (e) => {
    setLandmarkSelected(landmarkRef.current.selectedIndex);
    console.log(landmarkRef.current.selectedIndex);
  };

  return (
    <div id="main">
      <select name="state" id="state" ref={StateRef} onChange={handleStates}>
        <option value={0}>{states[0].name}</option>
        <option value={1}>{states[1].name}</option>
        <option value={2}>{states[2].name}</option>
        <option value={3}>{states[3].name}</option>
      </select>
      <select
        name="city"
        id="city"
        ref={cityRef}
        value={states[stateSelected].city[citySelected].name}
        onChange={handleCities}
      >
        <option value={0} key={"city0"}>
          {states[stateSelected].city[0].name}
        </option>
        <option value={1} key={"city1"}>
          {states[stateSelected].city[1].name}
        </option>
        <option value={2} key={"city2"}>
          {states[stateSelected].city[2].name}
        </option>
      </select>
      <select
        name="landmark"
        ...
        value={
          states[stateSelected].city[citySelected].landmarks[LandmarkSelected]
            .name
        }
      >
        {states[stateSelected].city[citySelected].landmarks.map((x, i) => {
          return (
